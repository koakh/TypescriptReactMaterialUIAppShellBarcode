...
import { defaultDrawerListItemIcon } from '../../config/constants';
...
export default function ResponsiveDrawer(props: ResponsiveDrawerProps) {
  ...
  let key = -1;
  drawerSections.forEach((section, index) => {
  ...
  const drawer = (
    <div>
      ...
      {/* { drawerSections.map((section: DrawerCategories[]) => <pre>{JSON.stringify(section, undefined, 2)}</pre>)} */}
      {/* {drawerSections.map((section: DrawerCategories[]) => {
        return (
          section.map((s, index) => (
            <ListItem button key={s.path}>
              <ListItemIcon>{index % 2 === 0 ? <InboxIcon /> : <MailIcon />}</ListItemIcon>
              <ListItemText primary={s.label + ':' + s.section} />
            </ListItem>
          )
          )
        )
